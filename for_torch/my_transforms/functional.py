...
def resize_by_cv2(pil_img, size, pil_interpolation):
    img = np.array(pil_img)
#   print(">> using opencv resize ......")
    img = cv2.resize(img, size, interpolation=pil2cv2_map[pil_interpolation])
...
#           return img.resize((ow, oh), interpolation)
            return resize_by_cv2(img, (ow, oh), interpolation)
        else:
            oh = size
            ow = int(size * w / h)
#           return img.resize((ow, oh), interpolation)
...
#       return img.resize(size[::-1], interpolation)
        return resize_by_cv2(img, size[::-1], interpolation)
